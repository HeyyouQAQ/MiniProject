    ...
    try {
        ...
        if (isset($data['systemConfig'])) {
            ...
            if ($check->num_rows > 0) {
                ...
                $stmt = $conn->prepare("UPDATE SystemConfiguration SET DefaultAnnualLeaveDays=?, PayrollCycleDay=?, QR_TOKEN_Export_Mins=?, MinimumShiftBreakMins=?, LatePenaltyAmount=?, LastModifiedBy=? WHERE ConfigID=?");
                if (!$stmt) {
                    throw new Exception("Prepare failed: " . $conn->error);
                }
                // types: i (annual), i (payroll), i (qr), i (break), d (penalty - double), i (modifier), i (confId)
                ...
                if (!$stmt->execute()) {
                 ...
                 // Insert
                 $stmt = $conn->prepare("INSERT INTO SystemConfiguration (DefaultAnnualLeaveDays, PayrollCycleDay, QR_TOKEN_Export_Mins, MinimumShiftBreakMins, LatePenaltyAmount, LastModifiedBy) VALUES (?, ?, ?, ?, ?, ?)");
                 if (!$stmt) {
                    throw new Exception("Prepare failed: " . $conn->error);
                 }
                 // types: i, i, i, i, d, i
